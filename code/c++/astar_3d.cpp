...
//            std::chrono::steady_clock::time_point  t3 = std::chrono::steady_clock::now();
            //cout<<newnode<<endl;
            bool not_in=cost_so_far.find(newnode) ==cost_so_far.end();
            if (not_in||(!not_in)&&newcost<cost_so_far[newnode])
            {//0.22
...
//                time_count2+=time_span2.count();
                newnodes[i].cost=newcost+ 2*H_cost(newnodes[i],size,dp);
...
//    std::cout << "It took me " << time_span1.count() << " seconds.";
    ...
    auto time_span2 = std::chrono::duration_cast<milliseconds>(t2 - t1);

    std::cout << "It took me" << time_span2.count()/pow(10,3) << " seconds.";
    std::cout << std::endl;
//    string s1="abc";
//    string s2="ab";
////    string s1="abc";
